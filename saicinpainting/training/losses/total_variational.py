...
class TotalVariationLoss(torch.nn.Module):
    def __init__(self):
        ...
        self.conv_h.weight.data = torch.tensor([[[[1, -1]]]]).to(torch.float32)
        self.conv_v.weight.data = torch.tensor([[[[1], [-1]]]]).to(torch.float32)
    ...
    def forward(self, img):
        ...
        img_h = self.conv_h(img.to(torch.float32))
        img_v = self.conv_v(img.to(torch.float32))
